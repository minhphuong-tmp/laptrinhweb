...
const useRealtimeComments = (postId, currentUser, initialComments = []) => {
    ...
    useEffect(() => {
        ...
        const loadComments = async () => {
            ...
            try {
                ...
                // Load comments
                const commentsUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/comments?postId=eq.${postId}&order=created_at.asc`;
                ...
                if (commentsResponse.ok) {
                    ...
                    if (commentIds && currentUser) {
                        ...
                        const likesUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/commentLikes?commentId=in.(${commentIds})&select=commentId`;
                        ...
                        if (likesResponse.ok) {
                            ...
                            const userLikesUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/commentLikes?commentId=in.(${commentIds})&userId=eq.${currentUser.id}&select=commentId`;
    ...
    const addComment = useCallback(async (content, image = null, video = null) => {
        ...
        try {
            ...
            const response = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/comments', {
                method: 'POST',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
    ...
    const likeComment = useCallback(async (commentId) => {
        try {
            ...
            if (isLiked) {
                ...
                const deleteUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/commentLikes?commentId=eq.${commentId}&userId=eq.${currentUser.id}`;
                ...
                const insertUrl = 'https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/commentLikes';
    ...
    const deleteComment = useCallback(async (commentId) => {
        try {
            ...
            const response = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/comments?id=eq.${commentId}`, {
                method: 'DELETE',
                headers: {
                    'apikey': apiKey,
                    'Authorization': `Bearer ${apiKey}`,
