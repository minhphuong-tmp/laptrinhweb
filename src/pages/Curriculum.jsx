...
import GradesCrawler from '../components/GradesCrawler';
import { convertScoreToLetter } from '../utils/gradeUtils';
import { requestStudySupport } from '../services/studyService';
...
const Curriculum = () => {
    ...
    const [confirmModal, setConfirmModal] = useState(null); // { subjectName, letterGrade }
    ...
    const handleConfirmSupport = async () => {
        ...
        setSubmitting(true);
        try {
            const result = await requestStudySupport(user?.id, confirmModal.subjectName, confirmModal.letterGrade);
            setConfirmModal(null);
            if (result?.error) {
                showToast('❌ Lỗi: ' + (result.error.message || 'Không thể gửi yêu cầu'), 'error');
            } else {
                showToast('✅ Đã gửi yêu cầu hỗ trợ môn "' + confirmModal.subjectName + '"!', 'success');
            }
        } catch (err) {
            setConfirmModal(null);
    ...
    const handleEdit = () => {
        const initial = cachedGrades ? JSON.parse(JSON.stringify(cachedGrades)) : [];
    ...
    const handleGradeChange = (rowIndex, colIndex, val) => {
        setTempGrades(prev => {
            ...
            // Auto-calculate letter grade if Summary Score (TK - col 9) is changed
            if (colIndex === 9) {
                newGrades[rowIndex][10] = convertScoreToLetter(val);
            }
    ...
    const handleAddGrade = (courseStr, colIndex, val) => {
        const match = courseStr.match(/^(.*?)\s*\((\d+)\s*tín chỉ\)$/);
        const subjectName = match ? match[1] : courseStr;
        const credits = match ? match[2] : '0';

        const newRow = new Array(16).fill('');
        newRow[0] = `manual-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`; // Unique ID
        newRow[3] = subjectName;
        newRow[11] = credits.toString();
        newRow[colIndex] = val;

        // Auto-calculate letter grade if Summary Score (TK - col 9) is changed
        if (colIndex === 9) {
            newRow[10] = convertScoreToLetter(val);
        }

        newRow[15] = 'true'; // Manual edit flag

        setTempGrades(prev => [...prev, newRow]);
    };
    ...
    const { roadmap, mapSubjectToGrade, unmapped, stats, displayGrades } = useMemo(() => {
        ...
        return {
            ...
            mapSubjectToGrade,
            unmapped,
            stats: { totalCredits, learnedCredits, avgScore },
            displayGrades
        };
    }, [curriculumData, selectedMajor, isEditing, tempGrades, cachedGrades]);
        ...
        <div className="page-content">
            <div className="curriculum-container">
                ...
                < div className="curriculum-section" >
                    ...
                    {/* Statistics and Roadmap Render */}
                    <div className="progress-stats">
                        <div className="stats-row">
                            <div className="stat-box">
                                <div className="stat-val">{stats.learnedCredits} (Đã học) / {stats.totalCredits} (Tổng)</div>
                    ...
                    {/* WARNING / SUMMARY */}
                    {!cachedGrades && (
                        <div className="no-grades-section">
                            <div className="no-grades-warning" style={{ marginBottom: '20px', padding: '15px', background: '#fff3cd', color: '#856404', borderRadius: '8px', border: '1px solid #ffeeba' }}>
                                ⚠️ Bạn chưa có dữ liệu điểm thi chính thức. Vui lòng đăng nhập để hệ thống tự động crawl điểm và cá nhân hóa lộ trình học tập của bạn.
                            </div>
                            <GradesCrawler
                            ...
                            <div style={{ marginTop: '30px', textAlign: 'center' }}>
                                <p style={{ color: '#718096', fontStyle: 'italic' }}>Dưới đây là lộ trình mẫu để bạn tham khảo:</p>
                            </div>
                        </div>
                    )}
                    {displayGrades && (
                    ...
                    <div className="progress-roadmap">
                                                ...
                                                {semester.courses.map((courseStr, cIndex) => {
                                                    ...
                                                    const renderInput = (colIdx, val) => (
                                                        <input
                                                            ...
                                                            onChange={(e) => {
                                                                if (gradeRow) {
                                                                    handleGradeChange(gradeIndex, colIdx, e.target.value);
                                                                } else {
                                                                    handleAddGrade(courseStr, colIdx, e.target.value);
                                                                }
                                                            }}
                                                    ...
                                                    return (
                                                        <tr key={cIndex} className={`grade-row ${statusClass}`} style={gradeRow && gradeRow[15] === 'true' ? { backgroundColor: '#fffaf0' } : {}}>
                                                            <td className="col-subject">
                                                                {subjectName}
                                                                {isNonGPA && <sup style={{ color: 'red', marginLeft: '2px' }}>*</sup>}
                                                            </td>
                                                            <td className="col-credits">{credits}</td>
                                                            <td className="col-score">{isEditing ? renderInput(5, gradeRow ? gradeRow[5] : '') : (gradeRow ? gradeRow[5] : '-')}</td>
                                                            <td className="col-score">{isEditing ? renderInput(6, gradeRow ? gradeRow[6] : '') : (gradeRow ? gradeRow[6] : '-')}</td>
                                                            <td className="col-score">{isEditing ? renderInput(8, gradeRow ? gradeRow[8] : '') : (gradeRow ? gradeRow[8] : '-')}</td>
                                                            <td className="col-score font-bold">{isEditing ? renderInput(9, gradeRow ? gradeRow[9] : '') : (gradeRow ? gradeRow[9] : '-')}</td>
                                                            ...
                                                            <td className="col-action">
                                                                {isFail || ['D', 'D+'].includes(letterGrade) ? (
                                                                    <button
                                                                        className="support-request-btn-small"
                                                                        onClick={() => setConfirmModal({ subjectName, letterGrade })}
                                                    ...
                                                    <tr key={idx} className={`grade-row ${statusClass}`} style={row[15] === 'true' ? { backgroundColor: '#fffaf0' } : {}}>
                                                        ...
                                                        <td className="col-action">
                                                            {isFail || ['D', 'D+'].includes(letterGrade) ? (
                                                                <button
                                                                    className="support-request-btn-small"
                                                                    onClick={() => setConfirmModal({ subjectName: row[3], letterGrade })}
                                                                >
                                                                    🤝
                                                                </button>
                                                            ) : '-'}
                        ...
                        <div className="roadmap-note" style={{ marginTop: '20px', textAlign: 'right', fontStyle: 'italic', color: '#718096', fontSize: '0.9rem' }}>
                            * : Môn học không tính vào điểm trung bình (Giáo dục thể chất, Giáo dục quốc phòng)
                        </div>
                    </div >
                </div >
            ...
            {/* GPA Simulator Modal */}
            {
                showSimulator && (
                    <GPASimulator
                        ...
                        roadmap={roadmap}
                        mapSubjectToGrade={mapSubjectToGrade}
                        unmapped={unmapped}
            ...
            {confirmModal && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ color: '#1e293b', marginBottom: '8px', fontSize: '1.2rem' }}>
                            Xác nhẫn đăng ký hỗ trợ
                        </h3>
                        <p style={{ color: '#64748b', marginBottom: '20px', lineHeight: '1.6' }}>
                            Bạn muốn đăng ký hỗ trợ học tập cho môn<br />
                            <strong style={{ color: '#3b82f6', fontSize: '1.1rem' }}>{confirmModal.subjectName}</strong>
                            {confirmModal.letterGrade && (
                                <><br /><span style={{ color: '#ef4444', fontWeight: 600 }}>(Điểm: {confirmModal.letterGrade})</span></>
                            )}
                            <br /><br />
                            <span style={{ fontSize: '0.85rem', color: '#94a3b8' }}>
                                Admin sẽ liên hệ và sắp xếp buổi phụ đạo sớm nhất có thể.
                            </span>
                        </p>
                        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontSize: '0.95rem', opacity: submitting ? 0.7 : 1,
                                }}
                            >
            ...
            {toast && (
                <div style={{
                    ...
                    <button
                        ...
                        style={{ background: 'none', border: 'none', color: 'white', cursor: 'pointer', fontSize: '1.2rem', padding: '0 0 0 6px', lineHeight: 1 }}
                    >×</button>
                </div>
            )}
        </div >
