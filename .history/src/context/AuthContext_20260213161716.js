...
export const AuthProvider = ({ children }) => {
    ...
    const signIn = async (email, password) => {
        ...
        try {
            ...
            // data.session contains access_token and user
            const authData = data.session;
            const user = data.user;

            // Lưu token vào localStorage (Supabase client auto-persists, but we keep this for compatibility if needed)
            // Note: Supabase client handles persistence automatically via 'sb-tguxydfhxcmqvcrenqbl-auth-token' if configured in lib/supabase.js
            // But we'll manually set our detailed user state.
            ...
            if (user) {
                ...
                try {
                    ...
                    if (userDataResult.success && userDataResult.data) {
                        ...
                        // Merge basic auth data with db data to ensure we don't lose auth info
                        setUser({ ...basicUser, ...userDataResult.data });
                    } else {
                        console.log('⚠️ Failed to load user data from database, keeping basic user');
                        // Thử sync user với auth nếu chưa có trong database
                        const syncResult = await syncUserWithAuth(user.id);
    ...
    const signOut = async () => {
        try {
            ...
            // Clear localStorage
            localStorage.removeItem('sb-tguxydfhxcmqvcrenqbl-auth-token');
