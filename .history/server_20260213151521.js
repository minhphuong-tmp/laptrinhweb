...
app.post('/api/crawl-schedule', async (req, res) => {
    ...
    console.log(`Starting SuperKMA Schedule Crawler for: ${studentId}`);
    let browser = null;
    ...
    try {
        ...
        // 1. Navigate to SuperKMA Auth
        console.log('Navigating to SuperKMA Auth...');
        await page.goto('https://schedule.superkma.com/auth', {
            waitUntil: 'networkidle2',
            timeout: 60000
        });

        // 2. Click ACTVN button
        console.log('Looking for ACTVN login button...');
        await page.waitForSelector('button, a, div[role="button"], span', { timeout: 5000 }).catch(() => { });

        const clicked = await page.evaluate(() => {
            const buttons = Array.from(document.querySelectorAll('button, a, div[role="button"], span'));
            const actvnBtn = buttons.find(btn => {
                const text = (btn.textContent || btn.innerText || '').toLowerCase();
                return text.includes('actvn') || text.includes('học viện');
            });
        ...
        if (!clicked) {
            throw new Error('Không tìm thấy nút đăng nhập Học viện (ACTVN)');
        }

        // 3. Handle login inputs
        console.log('Waiting for login inputs...');
        await page.waitForSelector('input', { timeout: 15000 });
        const inputs = await page.$$('input');

        if (inputs.length < 2) {
            throw new Error('Không tìm thấy trường nhập liệu đăng nhập');
        }
        ...
        // 4. Wait for Schedule Data
        console.log('Waiting for schedule data...');
        // Wait for URL change or specific element
        try {
            await page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 10000 });
        } catch (e) {
            console.log('Navigation wait timeout, checking content anyway...');
        }

        // 5. Extract Details
        const scheduleData = await page.evaluate(() => {
            const data = [];
            // Strategy: Look for specific event cards or rows
            // Since structure varies, we grab generic text blocks that look like classes
            // For now, simple text extraction
            const elements = document.querySelectorAll('.fc-event, [class*="event"], td');

            elements.forEach(el => {
                const text = el.innerText;
                if (text && text.length > 5) data.push(text);
            });
            return data;
        });

        console.log(`Extracted ${scheduleData.length} raw items.`);

        await browser.close();
        res.json({ success: true, data: scheduleData });

    } catch (error) {
        console.error('SuperKMA Crawl Error:', error);
