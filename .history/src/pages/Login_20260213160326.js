...
const Login = () => {
    ...
    const handleQldtSubmit = async (e) => {
        e.preventDefault();
        const studentId = e.target.elements['qldt-user'].value;
        const password = e.target.elements['qldt-pass'].value;
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(data.error || 'Đăng nhập QLĐT thất bại');
            }
            ...
            const qldtUser = {
                id: 'qldt_' + studentId,
                email: `${studentId}@student.actvn.edu.vn`,
                name: studentId,
                type: 'qldt',
            ...
            const sessionData = {
                user: {
                    id: qldtUser.id,
                    email: qldtUser.email,
                    user_metadata: { name: qldtUser.name },
                },
                access_token: 'qldt-mock-token-' + Date.now()
            };
            localStorage.setItem('sb-tguxydfhxcmqvcrenqbl-auth-token', JSON.stringify(sessionData));

            // 3. Cache Schedule
            if (data.data) {
                localStorage.setItem('studentSchedule', JSON.stringify(data.data));

                // Also trigger the callback if passed (for Activities page)
                // We use a custom event or direct callback if props exist, 
                // but since this is the main login page, we just redirect.
                // However, Activities.jsx listens to 'qldt_sched_login'.
                const loginData = {
                    studentId,
                    scheduleData: data.data,
                    timestamp: Date.now()
                };
                localStorage.setItem('qldt_sched_login', JSON.stringify(loginData));
            }

            // 4. Set State & Navigate
            // setAuth(qldtUser); // If using AuthContext
            // But since we are using a dual-login system context might confuse Supabase user vs QLDT user.
            // For now, valid QLDT login redirects to Home or Activities.
            // If the user wants to use QLDT as main auth:
            if (typeof setAuth === 'function') setAuth(qldtUser);

            navigate('/activities');

        } catch (err) {
            console.error(err);
            setError(err.message || 'Lỗi kết nối đến Server QLĐT');
        } finally {
            setLoading(false);
        }
    };
    ...
    const fillSampleCredentials = () => {
        if (loginMethod === 'microsoft') {
            setEmail('ct060419@student.actvn.edu.vn');
            setPassword('Aa@123456');
        } else if (loginMethod === 'qldt') {
            // Placeholder for QLDT
            // alert('Chức năng QLĐT đang bảo trì');
            const qldtUser = document.getElementById('qldt-user');
            const qldtPass = document.getElementById('qldt-pass');
            if (qldtUser) qldtUser.value = 'CT060419';
            if (qldtPass) qldtPass.value = '123456';
        } else {
            setEmail('admin@actvn.edu.vn');
            setPassword('123456');
        }
    };
