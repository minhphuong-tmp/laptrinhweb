...
export const createConversation = async (data) => {
    try {
        ...
        const response = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
...
export const getConversations = async (userId) => {
    try {
        ...
        const membersUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?user_id=eq.${userId}&select=conversation_id,last_read_at,conversation:conversations(id,name,type,created_at,updated_at,created_by)`;
        ...
        const conversationsWithMessages = await Promise.all(
            membersData.map(async (item) => {
                // Lấy tin nhắn cuối
                const messagesUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/messages?conversation_id=eq.${item.conversation_id}&select=id,content,message_type,file_url,created_at,sender_id,sender:users(id,name,image)&order=created_at.desc&limit=1`;
                ...
                const conversationMembersUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${item.conversation_id}&select=user_id,last_read_at,is_admin,user:users(*)`;
...
export const getConversationById = async (conversationId) => {
    try {
        ...
        const conversationUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations?id=eq.${conversationId}&select=*`;
        ...
        const membersUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${conversationId}&select=user_id,last_read_at,is_admin,user:users(*)`;
        const membersResponse = await fetch(membersUrl, {
            method: 'GET',
            headers: {
                'apikey': apiKey,
...
export const addMemberToConversation = async (conversationId, userId) => {
    try {
        ...
        const response = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const removeMemberFromConversation = async (conversationId, userId) => {
    try {
        ...
        const response = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${conversationId}&user_id=eq.${userId}`, {
            method: 'DELETE',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const sendMessage = async (data) => {
    try {
        ...
        const response = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/messages', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
        ...
        await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations?id=eq.${data.conversation_id}`, {
            method: 'PATCH',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const getMessages = async (conversationId, limit = 50, offset = 0) => {
    try {
        ...
        const messagesUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/messages?conversation_id=eq.${conversationId}&select=*,sender:users(*),message_reads(user_id,read_at)&order=created_at.desc&limit=${limit}&offset=${offset}`;
...
export const markMessageAsRead = async (messageId, userId) => {
    try {
        ...
        const response = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/message_reads', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const editMessage = async (messageId, content) => {
    try {
        ...
        const response = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/messages?id=eq.${messageId}`, {
            method: 'PATCH',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const deleteConversation = async (conversationId, userId) => {
    try {
        ...
        const memberUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${conversationId}&user_id=eq.${userId}&select=is_admin,conversation:conversations(type)`;
        ...
        const messagesResponse = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/messages?conversation_id=eq.${conversationId}`, {
            method: 'DELETE',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
        ...
        const membersResponse = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${conversationId}`, {
            method: 'DELETE',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
        ...
        // Xóa conversation
        const conversationResponse = await fetch(`https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations?id=eq.${conversationId}`, {
            method: 'DELETE',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
...
export const createDirectConversation = async (userId1, userId2) => {
    try {
        ...
        const existingUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations?type=eq.direct&select=id,conversation_members!inner(user_id)`;
        ...
        if (existingResponse.ok) {
            ...
            if (existingConversations && existingConversations.length > 0) {
                for (const conv of existingConversations) {
                    const membersUrl = `https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members?conversation_id=eq.${conv.id}&select=user_id`;
                    const membersResponse = await fetch(membersUrl, {
                        method: 'GET',
                        headers: {
                            'apikey': apiKey,
        ...
        const createResponse = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversations', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
        ...
        const addMembersResponse = await fetch('https://tguxydfhxcmqvcrenqbl.supabase.co/rest/v1/conversation_members', {
            method: 'POST',
            headers: {
                'apikey': apiKey,
                'Authorization': `Bearer ${apiKey}`,
