                                                ...
                                                <table>
                                                    ...
                                                    <tbody>
                                                        {group.items.map((row, idx) => {
                                                            ...
                                                            const isLowGrade = ['D', 'D+', 'F'].includes(gradeLetter);

                                                            return (
                                                                <tr key={idx} className={`grade-${gradeLetter}`}>
                                                                    <td>{subjectName}</td> {/* Môn */}
                                                                    ...
                                                                    <td>
                                                                        {isLowGrade && (
                                                                            <button
                                                                                className="support-request-btn"
                                                                                onClick={async () => {
                                                                                    if (window.confirm(`Bạn muốn đăng ký hỗ trợ học tập môn ${subjectName}?`)) {
                                                                                        const { error } = await requestStudySupport(user.id, subjectName, gradeLetter);
                                                                                        if (error) alert('Lỗi: ' + error.message);
                                                                                        else alert('Đã gửi yêu cầu hỗ trợ thành công!');
                                                                                    }
                                                                                }}
                                                                            >
                                                                                🤝 Cần hỗ trợ
